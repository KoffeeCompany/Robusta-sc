...
async function main() {
  ...
  const pokeMe = await ethers.getContractAt("IPokeMe", addresses.PokeMe, user);
  ...
  const checkerResult = await resolver.checker(tokenId, optionData);

  console.log("Can Exec : ", checkerResult.canExec);

  const receipt = await pokeMe
    .connect(gelato)
    .exec(
      0,
      addresses.WETH,
      ...
      await pokeMe.getResolverHash(
        resolver.address,
        resolver.interface.encodeFunctionData("checker", [tokenId, optionData])
      ),
      option.address,
