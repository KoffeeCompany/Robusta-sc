...
import { IUniswapV3Factory, ISwapRouter, IERC20 } from "../../typechain";
...
async function main() {
  ...
  // #region Tokens addresses

  console.log("WETH : ", addresses.WETH);
  console.log("DAI : ", addresses.DAI);

  // #endregion Token addresses

  // #region Pool addresses

  const pool = await uniFactory.getPool(addresses.DAI, addresses.WETH, 500);
  console.log("Token 0 (DAI): ", addresses.DAI);
  console.log("Token 0 (WETH): ", addresses.WETH);
  console.log("Fee : 500");
  console.log("Pool addresses : ", pool);

  // #endregion Pool addresses
  ...
  expect(userAddress.toLowerCase()).to.be.eq(
    "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266".toLowerCase()
  );
  console.log("Use address : ", userAddress);
  console.log("Ether balance : ", (await user.getBalance()).toString());
  console.log("DAI balance : ", (await DAI.balanceOf(userAddress)).toString());

  // #endregion Get some DAI
}
